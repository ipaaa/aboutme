...
- DONE: Each of AC-1 through AC-8 is reproduced against the worktree HEAD with concrete evidence (paste actual grep output, rendered DOM observations, and diff stat).
  All eight ACs PASS at HEAD bf4eee9. AC-1: index.html:26 `<p>Contact me at: <a href="mailto:ipawei@gmail.com">ipawei@gmail.com</a></p>`. AC-2: index1.html:684 inside `<mark class="highlight-gray">` has `<a href="mailto:ipawei@gmail.com">ipawei@gmail.com</a><br/><a href="mailto:ipawei@proton.me">ipawei@proton.me</a><br/>`. AC-3: index.html:28 Twitter `href="https://x.com/ipa"`, Facebook unchanged at `https://www.facebook.com/ipa.chiu/`. AC-4: index1.html:685 FACEBOOK→`https://facebook.com/ipa.chiu/`, MEDIUM→`https://medium.com/@ipaway`, MASTODON→`https://g0v.social/@ipa`, TWITTER→`https://x.com/ipa` (paths unchanged). AC-5: both `g0v.tw` inline anchors (lines 688 and 691) use `href="https://g0v.tw"`. `git diff --stat main`: `index.html | 4 ++--`, `index1.html | 8 ++++----`.
- DONE: AC-6 and AC-7 independently re-run: `grep -nF 'href="http://' index.html index1.html` and `grep -nF '[at]' index.html index1.html` must both return no matches; paste the exit codes.
  `grep -nF 'href="http://' index.html index1.html` → 0 matches, exit 1 (EXIT_HTTP=1). `grep -nF '[at]' index.html index1.html` → 0 matches, exit 1 (EXIT_AT=1). Positive check `grep -nE 'href="mailto:ipawei@(gmail|proton)\.(com|me)' index.html index1.html` → 3 matches, exit 0 (index.html:26 + index1.html:684 gmail + index1.html:684 proton).
- DONE: AC-8 (diff bounded to expected text/href swaps only) verified by reading `git diff main` and confirming no CSS rules, no element add/remove, no class attribute changes, no whitespace reflow of unrelated lines.
  `git diff main -- index.html index1.html` shows only the eight named byte-range swaps from the AC tables. index.html: two lines changed (mailto conversion + Twitter scheme). index1.html: four lines changed (684 contact mailto pair, 685 four social schemes, 688 g0v.tw scheme, 691 g0v.tw scheme). SVG XML namespace `xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22` on line 681 appears unchanged in diff context. No `<style>`/CSS rule modified, no element added or removed, no class attribute changed, no `<br/>` reorder, no `<mark>` wrapper modified, no whitespace reflow. `git diff main -- ':!index.html' ':!index1.html'` excluding entity-file scaffolding changes shows no other source files touched.

### Summary

All eight acceptance criteria PASS against worktree HEAD bf4eee9. Both grep regression guards (AC-6 `href="http://`, AC-7 `[at]`) return exit code 1 with zero matches; positive mailto check returns three matches as expected. Diff scope is bounded: only the eight named text/href swaps appear in `git diff main -- index.html index1.html`; no CSS, structure, class, whitespace, or wrapper changes. Click-behavior items 4–9 in the test plan would normally require a live browser; verification is source-level only (every contact `href` starts with `mailto:`, every outbound social `href` starts with `https://`), which proves the navigation target but does not directly observe the OS mail-handler chooser or the browser URL bar. Gate-approval to `done`.
