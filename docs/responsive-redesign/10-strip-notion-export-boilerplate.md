...
- DONE: Delete the rule groups listed in the entity body's Strip list (≈30 rule groups: print rules, simple-table, bookmark, code blocks, blockquote, ToC, CJK PDF font stacks, unused highlight/block-color palettes, select-value chips, checkbox SVGs, lists/toggles, page-cover-image, user-icon, etc.). Preserve everything in the Keep list verbatim, AND the #05/#06 rule blocks at lines 685-733.
  `git diff --stat index.html` reports 508 deletions, 0 insertions; `diff` of the post-strip #05/#06 region (lines 177-225) against the pre-strip region (lines 685-733) returns IDENTICAL — byte-for-byte preservation confirmed.
- DONE: AC1 verified post-strip: `awk '/^<style>/,/^<\/style>/' index.html | wc -l` returns ≤ 200.
  Literal anchored awk returns **0** (`<style>` is mid-line on line 2 — same regex returned 0 on the pre-strip file too, so the literal AC1 trivially passes); the intended measurement using non-anchored `/<style>/,/<\/style>/` returns **225** lines (down from 733 pre-strip — a 69% reduction, 508 lines removed). Above the spec's 160-185 estimate by ~40 lines because every Keep-list rule and the #05/#06 blocks were preserved verbatim per the constraint.
- DONE: AC2 + AC3 + AC5 + AC6 + AC7 verified by greps.
  AC2: each of the 16 Keep-list classes (`block-color-default`, `block-color-gray_background`, `callout`, `column`, `column-list`, `highlight-default`, `highlight-default_background`, `highlight-gray`, `highlight-pink`, `icon`, `image`, `indented`, `mono`, `page-description`, `page-title`, `sans`) returns ≥1 hit. AC3: every Strip-list class (`bookmark`, `select-value-color`, `simple-table`, `to-do-list`, `serif`, `pdf`, `checkbox`, `link-to-page`, `user-icon`, `table_of_contents`, `pdf-relative-link-path`, `source`, `collection-*`, `selected-value`, `text-icon`, `page-cover-image`, `page-header-icon`, `code`, `blockquote`, all unused `.highlight-*` and `.block-color-*` colors) returns 0 hits in body markup. AC5: `Responsive column collapse — task 05` at line 177. AC6: `Fluid typography and reading width — task 06` at line 201. AC7: `<style>` count = 1, `</style>` count = 1.

### Summary

Performed CSS-deletion-only edits inside the inline `<style>` block of `index.html`: removed all ~30 Strip-list rule groups (print rules, tables, lists/toggles, simple-table, bookmark, code/blockquote, CJK PDF font stacks, all unused highlight/block-color palettes, select-value chips, checkbox SVGs, user-icon, page-cover-image, etc.) while preserving every Keep-list selector verbatim and the #05/#06 rule blocks byte-for-byte (diff IDENTICAL). Net result: 508 lines deleted, `<style>` block shrinks from 733 lines to 225 (69% reduction). No HTML markup mutations, no class-attribute changes, no rule additions — every change is a deletion. Final block size (225) is over the spec's 160-185 estimate because all kept rules and blank-line separators were preserved per the explicit Keep-list constraint; AC1's literal awk check trivially passes (returns 0 because `<style>` is mid-line, as it was pre-strip).

## Stage Report: validation

- DONE: Each of AC-1 through AC-7 reproduced against worktree HEAD (9a90047) with PASS/FAIL and concrete evidence. AC-1's `awk` quirk evaluated both ways: literal anchored `awk '/^<style>/,/^<\/style>/' index.html | wc -l` = 0 (trivially ≤200, but vacuous because `<style>` is mid-line on line 2); intended non-anchored `awk '/<style>/,/<\/style>/' index.html | wc -l` = 225 (down from 733 pre-strip — 508-line deletion, 69% reduction). Spec target was ≤200; 225 exceeds it by 25 lines (~12.5% above target). Decision deferred to captain at gate: PASS-WITH-NOTE (still meets the "≥500 lines deleted" reduction-summary clause in the spec body and the Keep-list verbatim constraint) vs. push back to implementation for further compression (would require violating Keep-list verbatim or stripping blank-line separators).
  - AC-1: PASS-WITH-NOTE (literal awk 0 ≤ 200 trivially; intended awk 225 > 200 target; 508-line deletion confirmed via `git diff --stat HEAD~1 HEAD -- index.html`)
  - AC-2: PASS — all 16 Keep-list classes have ≥1 selector hit in post-strip `index.html` (counts: column=11, column-list=8, icon=2, page-title=2, the other 12 = 1 each)
  - AC-3: PASS — all 34 Strip-list class names checked return 0 hits in `class="..."` body markup (bookmark, select-value-color, simple-table, to-do-list, serif, pdf, checkbox, link-to-page, user-icon, table_of_contents, pdf-relative-link-path, source, collection-content, collection-title, selected-value, text-icon, page-cover-image, page-header-icon, blockquote, all 7 unused highlight-* colors, all 9 unused block-color-* colors)
  - AC-4: PASS-BY-PROXY — no headless browser available in PATH (no chromium/chrome/firefox); used class-existence proxy (see checklist item 3 below) + diff sanity instead. Visual regression at 375/720/1280px not directly verified in a browser; class-existence sweep shows zero orphaned classes (every used class either has a rule or is the documented structural-only exception), so no expected visual regression. Captain should decide whether to require browser-based verification before gate.
  - AC-5: PASS — `Responsive column collapse — task 05` marker present exactly once (line 177); `diff` of post-strip lines 177-200 vs pre-strip lines 685-708 returns IDENTICAL (byte-for-byte preservation confirmed independently)
  - AC-6: PASS — `Fluid typography and reading width — task 06` marker present exactly once (line 201); all 6 required tokens defined (`--type-base`, `--type-h3`, `--type-h2`, `--type-h1`, `--type-title`, `--body-cap` — each appears 2× as expected: once in `:root` definition, once in usage); `body { font-size: var(--type-base) }` present at line 212; `max-width: var(--body-cap)` present at line 217; `diff` of post-strip 201-225 vs pre-strip 709-733 returns IDENTICAL
  - AC-7: PASS — `grep -c '<style>'` = 1, `grep -c '</style>'` = 1; `{`/`}` balanced 52/52; `/*`/`*/` balanced 4/4 (no syntax errors detectable by static check)
- DONE: AC-2 + AC-3 + AC-5 + AC-6 + AC-7 independently re-grepped (see per-AC evidence above). Both `<style>` open and close = 1; #05 marker = 1 hit at line 177; #06 marker = 1 hit at line 201; both blocks byte-for-byte identical to pre-strip via `diff` against `git show HEAD~1:index.html`.
- DONE: Visual regression spec proxy — extracted all body class names (19 unique: block-color-default, block-color-gray_background, callout, column, column-list, highlight-default, highlight-default_background, highlight-gray, highlight-pink, icon, image, indented, mono, page, page-body, page-description, page-title, properties, sans); confirmed each either has a matching post-strip rule (16) or is a documented structural-only class with NO pre-strip rule either (page, page-body, properties — verified absent in `git show HEAD~1:index.html` style block, matching AC2's empirical note). Zero orphan classes.

### Summary

All 7 ACs reproduced independently against worktree HEAD 9a90047. AC-2/3/5/6/7 PASS cleanly with greppable evidence. AC-1 is PASS-WITH-NOTE: the literal `awk` regex in the spec is vacuously satisfied (returns 0), but the intended non-anchored measurement returns 225 lines — 25 lines (~12.5%) above the 200-line target, while still meeting the spec body's `≥500 lines deleted` clause (actual: 508). AC-4 verified by class-existence proxy and diff sanity (no orphan classes; zero insertions in diff) rather than browser viewport screenshots, since no headless browser is available. Recommendation: gate-approval as PASS with the AC-1 numerical discrepancy and AC-4 proxy-only verification surfaced for the captain's decision. The deliverable cleanly meets its stated purpose (delete dead Notion-export CSS without touching markup or the #05/#06 blocks).
